...
<head>
  ...
  <title>Sign Up - Son Got Samples</title>
...
<body>
  <div class="auth-container">
    ...
    <form method="POST" action="{{ route('register.post') }}">
      ...
      {{-- NAME FIELD REMOVED - Only Email and Password --}}
    ...
    <div class="admin-section">
      <p>ADMIN</p>
      <button class="admin-quick-btn" onclick="window.location.href='{{ route('login.post') }}'">
...
<script>
    // URL se plan parameter lein
    const urlParams = new URLSearchParams(window.location.search);
    const selectedPlan = urlParams.get('plan');
    
    if (selectedPlan && (selectedPlan === 'web' || selectedPlan === 'full')) {
        document.getElementById('selected_plan').value = selectedPlan;
        
        // Show message
        const message = document.createElement('div');
        message.style.cssText = 'background: rgba(96,116,255,.1); border: 1px solid rgba(96,116,255,.3); border-radius: 8px; padding: 12px; margin-bottom: 20px; text-align: center;';
        message.innerHTML = '✨ You will be redirected to checkout after successful registration.';
